	...
	public void init() {
		...
		template.execute(new TransactionCallbackWithoutResult() {
			...
			protected void doInTransactionWithoutResult(final TransactionStatus status) {
				...
				final Loja casaDoCodigo = new Loja();
				casaDoCodigo.setNome("Casa do Código");

				em.persist(casaDoCodigo);

				final Loja musicalAlegre = new Loja();
				musicalAlegre.setNome("Musical Alegre");

				em.persist(musicalAlegre);

				final Categoria tecnologia = new Categoria("Tecnologia");
				em.persist(tecnologia);

				final Categoria musica = new Categoria("Música");
				em.persist(musica);

				final Produto cursoDeViolao = new Produto();
				...
				cursoDeViolao.setLinkDaFoto("12136ee913ed4f44860914d44650144e.png");

				em.persist(cursoDeViolao);

				final Produto livroDeArquitetura = new Produto();
				...
				livroDeArquitetura.setLinkDaFoto("capa-livro.png");

				em.persist(livroDeArquitetura);

				final Produto livroDeSpring = new Produto();
				...
				livroDeSpring.setLinkDaFoto("spring-framework-featured_large.png?v=1411567960");

				em.persist(livroDeSpring);

				final Produto violao = new Produto();
				...
				violao.setLinkDaFoto("violao.jpg");

				em.persist(violao);

				final Produto flauta = new Produto();
				...
				flauta.setLinkDaFoto("flauta-doce-germanicayamaha-yrs-23g-204013000.jpg");
